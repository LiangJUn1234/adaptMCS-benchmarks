"""Automated scaling benchmark runner for Phase 6C Milestone 2."""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run truth-only, biased DA, and corrected DA across scaling benchmarks"
    )
    ...
    parser.add_argument("--case30-line-outage-prob", type=float, default=0.015)
    parser.add_argument("--case57-line-outage-prob", type=float, default=0.012)
    parser.add_argument("--case118-line-outage-prob", type=float, default=0.01)
    parser.add_argument("--arm-timeout-sec", type=int, default=3600)
    parser.add_argument(
        "--output",
        default="results_scaling_summary.csv",
...
def _arm_definitions() -> List[Dict[str, Any]]:
    # 🚀 核心修改 2：单点打击，只跑注入了 Level 0 预筛的 corrected_da 模式
    return [
        {
            "arm_name": "corrected_da",
...
def _case_outage_map(args: argparse.Namespace) -> Dict[str, float]:
    return {
        "case30": float(args.case30_line_outage_prob),
        "case57": float(args.case57_line_outage_prob),
        "case118": float(args.case118_line_outage_prob),
    }


def _build_config(args: argparse.Namespace, *, line_outage_prob: float, arm: Mapping[str, Any]) -> Dict[str, Any]:
    cfg = ACConfig(
        N=int(args.N),
    ...
    cfg.update(
        {
            ...
            # 👇 核心修改：放宽代理模型的容错率，防止触发全量回退
            "level0_prescreen_K": 400,
            "level0_prescreen_tail_audit": 50,
        }
    )
    return cfg
...
def _success_row(
    *,
    ...
    return {
        ...
        "proxy_mode": str(config.get("proxy_mode")) if config.get("proxy_mode") is not None else None,
...
def main() -> None:
    ...
    outage_map = _case_outage_map(args)
    rows: List[Dict[str, Any]] = []

    print("Running automated scaling benchmarks")
    print(f"output={output_path}")

    for case_name in CASES:
        line_outage_prob = outage_map[case_name]
        print()
        print(f"[case] {case_name} line_outage_prob={line_outage_prob}")
        for arm in _arm_definitions():
            cfg = _build_config(args, line_outage_prob=line_outage_prob, arm=arm)
            print(f"  [arm] {arm['arm_name']} seed={args.seed}")
            row = _run_arm_with_timeout(
                case_name=case_name,
            ...
            rows.append(row)
            print(
                "    status={status} pf_hat={pf_hat} acceptance={acc} truth_calls={truth_calls} proxy_calls={proxy_calls}".format(
                    status=row["status"],
                    pf_hat=row["pf_hat"],
                    acc=row["acceptance_rate"],
                    truth_calls=row["truth_calls"],
                    proxy_calls=row["proxy_calls"],
                )
            )
