function out = mp_run_fdxb_minimal(case_data, debug, ac_fail_as_violation)
%MP_RUN_FDXB_MINIMAL Minimal fast-decoupled AC PF wrapper with scalar scores.
%   out = MP_RUN_FDXB_MINIMAL(case_data, debug, ac_fail_as_violation)
%   accepts either a MATPOWER case name (e.g., 'case14') or case struct,
%   runs one FDXB AC power-flow, and returns scalar security scores.
%
...
mpopt = mpoption('verbose', 0, 'out.all', 0, 'pf.alg', 'FDXB');
