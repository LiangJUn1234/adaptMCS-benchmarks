...
## 23. Positioning A1-Guard Relative to ACOPF-Guided DCOPF Parameter Optimization

1. A1-Guard is not the final method.
- It is an objective-alignment step.
- It converts Level-0 auditing from ACOPF `s_any` rank preservation to ACOPF failure-membership preservation.
- It does not modify raw DCOPF, DA, ACOPF truth, or MATPOWER.

2. Why A1-Guard was necessary:
- legacy guard expanded `K` to `N` even when DCOPF already captured ACOPF failures.
- `failure_label` guard reduced mean `truth_calls` from `1320` to `577` while preserving `pf_hat`.
- This proves the previous bottleneck was guard-objective mismatch, not only DCOPF proxy weakness.

3. Why this does not complete the project:
- A1-Guard does not train DCOPF parameters.
- A1-Guard does not yet optimize DC approximation coefficients or bias terms.
- A1-Guard only creates a valid evaluation setting for the next stage.

4. Bridge to literature:
- Taheri & Molzahn optimize DC approximation / DCOPF parameters using AC/ACOPF teacher data.
- Our project adapts that principle from dispatch accuracy to rare-event failure screening.
- Their target is DCOPF or DCPF accuracy relative to ACOPF/ACPF.
- Our target is DCOPF-based rare-event screening, truth-call reduction, and guard-compatible top-K failure coverage.
- Therefore the next stage must train DCOPF-derived score/parameters, not stop at guard correction.

5. Recommended project sequence:
- A0: raw DCOPF diagnosis
- A1: objective-aligned Level-0 guard
- A2: ACOPF-guided DCOPF score-parameter training
- A3: wrapper-level `proxy_mode="dcopf_optimized"`
- A4: formulation-level coefficient/bias optimization inspired by Taheri & Molzahn
- C: ML-calibrated DCOPF as a later extension
